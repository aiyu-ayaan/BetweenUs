...
| `chat.message_updated` | `Message` | Broadcasts an edited message or content modification. Also how a thread root's `thread` summary (`replyCount`, `lastReplyAt`) changes when a reply is sent, deleted or expires. |
