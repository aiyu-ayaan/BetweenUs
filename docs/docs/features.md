...
- **Spoilers**: wrap text in `||double pipes||` and it draws as an obscured bar until somebody clicks or taps it. Every spoiler in a message opens together and stays open while the app runs. Notifications, reply quotes and the pinned list show a fixed-width `▒▒▒▒` in its place, so a spoiler is never printed where it cannot be clicked. `` `||code||` `` stays literal.
- **Code blocks with highlighting**: a fence's language (```` ```ts ````, ```` ```py ````, ```` ```sql ````, and about twenty aliases covering JS/TS, Python, Kotlin, Java, C/C++, C#, Go, Rust, Swift, shell, SQL, JSON, YAML, CSS and HTML) colours keywords, strings, numbers and comments in the theme's own colours. An untagged or unknown fence is plain monospace. It is a small in-tree lexer (`syntax.ts` / `Syntax.kt`) rather than a highlighting library, and it is client-only: the message is already decrypted, so nothing about it reaches the server.
- **Polls**: Ask a question with two to ten options, single or multi-choice, with an optional closing time. Live bars, counts and who-chose-what update as people vote; change or retract your vote; the author (or a moderator) closes it. The question and options are inside the encrypted message; the server only counts option indexes. A poll cannot be forwarded: its ballot belongs to the original message.
- **Edit history**: an edited message's "(edited)" marker opens a list of what it said before, newest first with timestamps - a popover on desktop and web, a bottom sheet on Android. Each earlier version is kept sealed under the same channel key as the message and opened only on your device, so the server learns no more about the old words than the new. Nothing is kept for a disappearing or one-time message, at most 50 versions are held, and deleting a message deletes its history.
- **Threads**: "Reply in thread" on any message opens a side conversation under it - a side panel on desktop and web, its own screen on Android. Replies stay out of the channel timeline; the root wears an "N replies · last reply X ago" chip. A thread is as private as its channel and sealed with the same key, so the server knows which message a reply hangs off and how many there are, never what was said. Only the people in a thread (its root's author and whoever has replied) are notified of a reply, and anyone else only when they are @mentioned. Deleting the root leaves the thread reachable under "Original message deleted"; a root that disappears takes its thread with it. Threads are one level deep and are not offered on one-time messages. The thread composer takes files the same way the channel composer does, sealed under the channel key. You follow a thread you start or reply to (or with *Follow* in its panel), and a followed thread's chip shows how many replies you have not seen, cleared on every device once you have read them; desktop and web list your followed threads in a side panel, and Android in its own screen (from the channel menu, the drawer header or Friends, with a badge of unread replies), where the thread screen also has the Follow / Unfollow button and takes files in a reply.
...
- **Pinned and search paging**: The pinned list loads 25 at a time and pages in as you scroll. Search matches the decrypted window at once, then walks back through older history on the device (older ciphertext pages are fetched with the message cursor and decrypted locally), showing how far back it has read, with a Stop button and a 1000-message cap per run before you choose to continue. The server never searches message text.
...
- **My Moments**: Everything you have posted, on every client, one tile per moment with its own age, view count and reaction tally. The grid reads newest first while the player still plays a run oldest to newest — the order it was lived in. On Android a drag up inside the player opens the same list of who watched.
