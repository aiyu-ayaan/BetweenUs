...
- **Followed threads** is a screen (`FollowedThreadsScreen`) opened from the channel menu, from the drawer's header (next to the switcher) and from the Friends header at home; both entries (`FollowedThreadsButton`) wear a badge of unread followed replies in their own scope (`ThreadRules.scopeUnread` over `Conversation.threadUnread` and `threadScope`, the server each followed root hangs off, null for direct messages). It shows the server on screen, or - at home - the direct messages, most recent first, from `GET /api/v1/messages/threads/followed`. Each root is decrypted on the device with its channel's key (`Conversation.loadFollowedList`); the row shows the root's preview, the reply count and an unread badge, and opens the thread. `ThreadRules` holds the pure parts (preview line, scope filter, unread badge). While the screen is open the list stays fresh: a `thread.follow` that adds or removes a thread, a reconnect, and `Conversation.resumeThreads` all call `refreshFollowedList()`, which reloads in place (rows and scroll stay until the answer; the newest request wins). `ThreadRules.shouldMarkSeen` is the pure gate for marking a thread read on resume.
