...
Password for both: `nexora-dev-pass`. They share the workspace **Duo Test** and
its `#general` channel.
...
**Calls**
- Click **Call** in one window, then in the other. Both tiles appear.
- The panel says *End-to-end encrypted*; if it does not, the join was aborted
  rather than downgraded to plaintext media.
...
ordering. It needs Postgres, Redis and the three services running.
...
| Call connects, no audio or video | LiveKit UDP ports 50000-50019 not published; check `docker compose -f infrastructure/docker/docker-compose.dev.yml ps` |
