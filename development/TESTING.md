...
| The installed desktop app opens a dialog saying "A JavaScript error occurred in the main process" and never opens a window | Something in `electron/main.ts` touched an Electron module at the top level that is only built after `ready` - `screen` was the one that did it. The throw comes out of the module's own getter while the entry is still evaluating, so there is nothing to catch it and no window to report it in, and it only shows up in a packaged build. Anything needing `ready` belongs inside the `app.whenReady()` handler |
...
| Thrown back to the login form after a lift, a sleep or a backend restart | Fixed on desktop, web and Android. Refreshing the access token used to end the session whenever the request failed, and only a 401 means the credential was actually refused. Every other failure now leaves a running session alone - the token is untouched and the next request refreshes again. If it still happens, the server really is rejecting the token: check auth-service logs for `Refresh tokens revoked` |
| The app goes quiet - no new messages, no online dots - but is still signed in, and a restart fixes it | Fixed on desktop, web and Android. Both sockets carry the access token in their URL, so it expires while the socket is open (15m, or any sleep or doze longer than that) and the reconnect was refused with 4401, which the socket treated as final. A 4401 now asks for a fresh token and reconnects. If it persists, watch for `closed code=4401` repeating in the client log: that is a refresh that is failing for its own reason |
