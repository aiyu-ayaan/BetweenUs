...
| Calls work on the LAN but not through the Cloudflare tunnel | Expected, and not fixable by configuration here. The tunnel carries HTTP, so signalling arrives; WebRTC media needs `7881/tcp` and `50000-50019/udp` reachable at `LIVEKIT_NODE_IP`. A public deployment needs those exposed directly or a TURN server |
