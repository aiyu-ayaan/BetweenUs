 ...
 * as the call lasts. On the main thread that competes with React, with the
 * message list rendering, and with every animation in the application - and the
 * symptom is not a slow filter, it is a call where the whole interface stutters
...
import { FRAME_BUDGET_MS, type CameraEffect } from './camera-effects';
...
async function run(message: StartMessage): Promise<void> {
  ...
  try {
    while (!stopping) {
      ...
      // Reported rather than acted on here: the decision to bypass is the main
      // thread's, because it is the side that owns the track swap. A worker
      // that quietly stopped filtering would leave the interface saying an
      // effect is on while it is not.
      if (took > FRAME_BUDGET_MS) self.postMessage({ type: 'slow', ms: took });
      else self.postMessage({ type: 'fast', ms: took });
    }
  } catch {
...
self.onmessage = (event: MessageEvent<Incoming>): void => {
  ...
  else if (message.type === 'effect') effect = message.effect;
  else if (message.type === 'stop') stopping = true;
};
