...
fun EmojiSection(serverId: String, mayManage: Boolean, onNote: (String?) -> Unit) {
    ...
    suspend fun reload() {
        emoji = runCatching { BetweenUsApi.serverEmoji(serverId) }.getOrNull().orEmpty()
    }
        ...
        scope.launch {
            ...
            onNote(
                runCatching {
                    ...
                    val stored = BetweenUsApi.uploadPicture(bytes, picked.name, picked.contentType)
                    BetweenUsApi.addServerEmoji(serverId, wanted, stored.url, animated)
                    name = ""
                    reload()
                    null
    ...
    val list = emoji
    when {
        list == null -> ListRow(title = "Loading emoji", leading = { BetweenUsIcon(BetweenUsIcons.Smile) })
        list.isEmpty() -> ListRow(
            title = "No emoji yet",
            subtitle = "Add one and it can be typed as :name:",
            leading = { BetweenUsIcon(BetweenUsIcons.Smile) },
        )
            ...
            ListRow(
                ...
                trailing = {
                    if (mayManage) {
                        IconAction(BetweenUsIcons.Trash, "Remove this emoji", tint = Danger, onClick = {
                            scope.launch {
                                busy = true
                                onNote(
                                    runCatching {
                                        BetweenUsApi.removeServerEmoji(serverId, one.id)
                                        reload()
                                    }.exceptionOrNull()?.message,
                                )
                                busy = false
