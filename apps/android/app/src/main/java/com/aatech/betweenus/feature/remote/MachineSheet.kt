...
import com.aatech.betweenus.core.data.REMOTE_PERMISSIONS
import com.aatech.betweenus.core.data.RemoteAuditEntry
import com.aatech.betweenus.core.data.RemoteGrant
...
fun MachineSheet(
    ...
    LaunchedEffect(machine.id) {
        grants = runCatching { BetweenUsApi.machineGrants(machine.id) }.getOrDefault(emptyList())
        audit = runCatching { BetweenUsApi.machineAudit(machine.id) }.getOrDefault(emptyList())
    }
    ...
    LaunchedEffect(query) {
        ...
        found = runCatching { BetweenUsApi.searchUsers(wanted) }
    ...
    fun set(userId: String, permissions: List<String>) {
        scope.launch {
            busy = true
            note = runCatching {
                grants = BetweenUsApi.setMachineGrant(machine.id, userId, permissions)
                audit = runCatching { BetweenUsApi.machineAudit(machine.id) }.getOrDefault(audit)
                query = ""
            ...
            BetweenUsButton(
                ...
                onClick = {
                    scope.launch {
                        busy = true
                        note = runCatching {
                            BetweenUsApi.renameMachine(machine.id, name.trim())
                            onChanged()
                        }.exceptionOrNull()?.message
                        busy = false
                    }
                    ...
                    Chip(
                        ...
                        onClick = {
                            scope.launch {
                                busy = true
                                note = runCatching {
                                    BetweenUsApi.removeMachine(machine.id)
                                    onChanged()
                                    onDismiss()
                                }.exceptionOrNull()?.message
                                busy = false
