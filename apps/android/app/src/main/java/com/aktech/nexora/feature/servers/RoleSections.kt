...
fun RoleSection(serverId: String, mayManage: Boolean, onNote: (String?) -> Unit) {
    ...
    suspend fun reload() {
        roles = runCatching { NexoraApi.serverRoles(serverId) }.getOrNull().orEmpty()
    }
    ...
    val list = roles
    when {
        list == null -> ListRow(title = "Loading roles", leading = { NexoraIcon(NexoraIcons.Shield) })
        list.isEmpty() -> ListRow(
            ...
            ListRow(
                ...
                leading = {
                    NexoraIcon(
                        icon = NexoraIcons.Shield,
                        tint = role.colour?.let { parseColour(it) } ?: Slate500,
                    )
                },
                trailing = {
                    if (mayManage) {
                        IconAction(NexoraIcons.Settings, "Edit this role", onClick = { editing = role })
                    }
                },
            ...
            NexoraButton(
                ...
                onClick = {
                    scope.launch {
                        ...
                        note = runCatching {
                            if (role == null) {
                                NexoraApi.createServerRole(serverId, name.trim(), colour, held.toList())
                            } else {
                                NexoraApi.updateServerRole(
                                    serverId = serverId,
                                    roleId = role.id,
                                    name = name.trim(),
                            ...
                            onClick = {
                                scope.launch {
                                    ...
                                    val failure = runCatching {
                                        NexoraApi.deleteServerRole(serverId, role.id)
                                        onChanged()
                                        onDismiss()
                                    }.exceptionOrNull()?.message
                                    onNote(failure)
