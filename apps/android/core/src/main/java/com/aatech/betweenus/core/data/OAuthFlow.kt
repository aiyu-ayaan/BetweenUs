 ...
 * thing, so the provider hand-off comes back through `betweenus://oauth`, and
...
object OAuthFlow {
    ...
    const val REDIRECT = "betweenus://oauth"
    ...
    fun init(context: Context) {
        prefs = context.applicationContext
            .getSharedPreferences("betweenus.oauth", Context.MODE_PRIVATE)
    }
    ...
    fun codeIn(uri: Uri?): String? {
        if (uri == null || uri.scheme != "betweenus" || uri.host != "oauth") return null
    ...
    suspend fun complete(code: String) {
        ...
        Session.start(BetweenUsApi.oauthExchange(code, verifier))
    }
