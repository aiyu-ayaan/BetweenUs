...
import com.aatech.betweenus.core.BuildConfig
 ...
 *
 * The port of `apps/desktop/src/services/endpoint.ts`. BetweenUs is meant to be
 * self-hosted, so the address the build shipped with is a default and not a
 * decision: [BuildConfig.DEFAULT_SERVER_URL] comes from `betweenus.serverUrl` in
...
object Endpoint {
    ...
    fun init(context: Context) {
        prefs = context.applicationContext.getSharedPreferences("betweenus.endpoint", Context.MODE_PRIVATE)
    }
     ...
     * public, always present, and answers with something recognisably BetweenUs.
     */
    ...
    suspend fun probe(base: String): String = withContext(Dispatchers.IO) {
        ...
        if (!result.body.trimStart().startsWith("[")) {
            throw IllegalArgumentException("That address is not a BetweenUs server")
        }
